...
public class DesktopExample {
    ...
    public static void main(String[] args) {
        // 初始化OpenCV（JavaCV 自动加载，这里只是确认）
        try {
            OpenCVInitializer.initialize();
            System.out.println("JavaCV OpenCV初始化成功");
        } catch (Exception e) {
            System.err.println("JavaCV OpenCV初始化失败: " + e.getMessage());
            return;
        }
    ...
    static void measureExample() {
        ...
        // 目标图像路径
        String targetImagePath = "/Users/xiongguochao/Desktop/针测试2.jpeg";
        ...
        try (NeedleLengthAnalyzer analyzer = new NeedleLengthAnalyzer(templatePath)) {
            ...
            System.out.println("针尖1: (" + result.getTip1X() + ", " + result.getTip1Y() + ")");
            System.out.println("针尖2: (" + result.getTip2X() + ", " + result.getTip2Y() + ")");
    ...
    static void measureFromMatExample() {
        ...
        // 从某处获取Mat（例如从相机、网络等）
        Mat targetImage = opencv_imgcodecs.imread("path/to/target.jpg");
        ...
        try (NeedleLengthAnalyzer analyzer = new NeedleLengthAnalyzer(templatePath)) {
            ...
            opencv_imgcodecs.imwrite("output/result_visualization.png", visualization);
            visualization.close();
